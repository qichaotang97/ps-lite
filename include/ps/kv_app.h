...
struct KVPairs {
    ...
    // KVPairs() {}
    // if lens exists, keys.len = lens.len
...
public:
    ...
    // ranges Sliced by the server
    // index: ranges.index
    // SlicedKVs[index].first: True, send SlicedKVs[index].second
    //                         False, not send SlicedKVs[index].second
...
private:
    ...
    // {(timestamp, std::vector<KVPairs<Val>>), ...}
...
/**
 * \brief an example handle adding pushed kv into store  server <--push-- worker, server --pull--> worker
 */
template <typename Val>
...
// simple_app类型消息，多是控制命令消息，用于系统中节点之间的协同工作，对应Request/Response操作；
// 非simple_app类型消息，多是Server与Worker之间的数据通信，数据封装在KVPairs中，对应Pull/Push操作；
// obj_ -> bind Process
// Process -> request_handle_ -> Response -> (Postoffice::Get()->van()->Send(msg)) 数据通信线路
//         -> SimpleApp::Process(msg); 控制通信线路
template <typename Val>
void KVServer<Val>::Process(const Message& msg) {
    // msg.meta.simple_app type message, mainly control command message
...
///////////////////////////////////////////////////////////////////////////////
// Worker节点启动过程与Scheduler、Server一样。唯一的区别在于，Worker启动时不需要自定义请求处理逻辑。
// 对于PS机器学习任务来讲，Worker端的行为都是主动行为，比如数据通信则主动向Server端执行Pull/Push请求，
// （控制）消息传输向Server端或Scheduler执行Request/Response请求。
// obj_ -> bind Process
// Process -> RunCallback(ts) 数据通信线路
//         -> SimpleApp::Process(msg); 控制通信线路
// Push/Pull -> AddCallback, Send -> slicer_, Postoffice::Get()->van()->Send(msg);
//
...
void KVWorker<Val>::Send(int timestamp, bool push, int cmd, const KVPairs<Val>& kvs) {
    // slice the message
    SlicedKVs sliced;
    // sliced.len = servers.len
    // sliced.index = servers.index
    // Postoffice::Get()->GetServerKeyRanges() : len(ranges) == len(servers)
    slicer_(kvs, Postoffice::Get()->GetServerKeyRanges(), &sliced);

    // need to add response first, since it will not always trigger the callback
    int skipped = 0;
    for (size_t i = 0; i < sliced.size(); ++i) {
        // sliced[i].first = false, not send, so skipped
        if (!sliced[i].first) {
            ++skipped;
        }
    }
    obj_->AddResponse(timestamp, skipped);
    // all sliced not send, all skipped, so trigger RunCallback
    if ((size_t)skipped == sliced.size()) {
        RunCallback(timestamp);
    }

    // sliced.size() == Server.size()
    // i == ServerRank
    for (size_t i = 0; i < sliced.size(); ++i) {
        const auto& s = sliced[i];
        // s.first = false, not send, skipped
        if (!s.first) {
            continue;
        }
        Message msg;
