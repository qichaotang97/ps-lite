...
void Van::ProcessAddNodeCommandAtScheduler(
    ...
    if (nodes->control.node.size() == num_nodes) {
        ...
        Message back;
        back.meta = *nodes;
        // send back message to kWorkerGroup + kServerGroup(all worker and server)
...
void Van::ProcessBarrierCommand(Message* msg) {
    ...
    if (msg->meta.request) {
        ...
        if (barrier_count_[group] ==
            ...
            // Uninitialized res.meta.control.barrier_group, will be random value
            // send back message to kWorkerGroup + kServerGroup + kScheduler(all worker and server and scheduler)
            for (int r : Postoffice::Get()->GetNodeIDs(group)) {
                int recver_id = r;
                if (shared_node_mapping_.find(r) == shared_node_mapping_.end()) {
                    res.meta.recver = recver_id;
                    res.meta.timestamp = timestamp_++;
                    // scheduler-> all worker and server and scheduler
                    CHECK_GT(Send(res), 0);
                }
            }
        }
    ...
    if (is_scheduler_) {
        ...
        for (const auto& node : ctrl.node) {
            ...
            if (connected_nodes_.find(addr_str) == connected_nodes_.end()) {
                Connect(node);    // 同种类型的节点, 不会进行连接成功, connected_nodes_ 存储的并不是连接成功的节点
...
void Van::Start(int customer_id) {
    ...
    if (init_stage == 0) {
        ...
        // debug receiver_thread_ id
        LOG(INFO) << "start news sub thread::receiver_thread_, function::Van::Receiving";
        ++init_stage;
    }
    start_mu_.unlock();
    ...
    if (init_stage == 1) {
        ...
        if (!is_scheduler_) {
            ...
            // debug heartbeat_thread_ id
            LOG(INFO) << "start news sub thread::heartbeat_thread_, function::Van::Heartbeat";
        }
        ++init_stage;
    }
    start_mu_.unlock();
...
void Van::Receiving() {
    ...
    while (true) {
        ...
        if (ready_.load() && drop_rate_ > 0) {
            unsigned seed = time(nullptr) + my_node_.id;
