...
protected:
    ...
    int SendMsg(const Message& msg) override {
        ...
        while (true) {
            if (zmq_msg_send(&meta_msg, p_socket, tag) == meta_size) {
                break;
            }
            if (errno == EINTR) {
                continue;
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (i == n - 1) tag = 0;
            while (true) {
                if (zmq_msg_send(&data_msg, p_socket, tag) == data_size) {
                    break;
                }
                if (errno == EINTR) {
...
private:
    ...
    /**
     * \brief node_id to the socket for sending data to this node, {(node_id, socket*), ...}
     */
