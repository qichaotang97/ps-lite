...
Customer::Customer(int app_id, int customer_id, const Customer::RecvHandle& recv_handle)
    ...
    // debug recv_thread_ id
    LOG(INFO) << "start news sub thread::recv_thread_, function::Customer::Receiving";
}
...
// Customer Receiving Thread receive message from Van Receiving Thread
// just after the Node's Van Object receive message, according to different message,
//     push to the different Customer Object
//     Message recv from void Van::Receiving() -> Van::ProcessDataMsg() -> obj->Accept(*msg);
//                            -> Customer::Receiving() -> recv_queue_.WaitAndPop(&recv) -> recv_handle_(recv);
